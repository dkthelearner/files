...
function configureSwagger(app): void {
    const config = new DocumentBuilder().setTitle('files').setDescription('API Description').setVersion('1.0').build();
...
async function bootstrap() {
    ...
    logger.log(`🚀 Files service running on port ${configService.get('servicePort')}`);
}
